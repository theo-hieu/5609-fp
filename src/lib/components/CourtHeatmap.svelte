<script lang="ts">
  ...
  function updateTooltipPosition(event: PointerEvent) {
    ...
    mousePos = {
      x: Math.min(Math.max(preferredX, padding), Math.max(padding, container.clientWidth - tooltipWidth - padding)),
      y:
        preferredY >= padding
          ? preferredY
          : Math.min(
              Math.max(fallbackY, padding),
              Math.max(padding, container.clientHeight - tooltipHeight - padding)
            )
    };
  }
  ...
  function drawCourt(layer: d3.Selection<SVGGElement, unknown, null, undefined>) {
    const stroke = '#f8fafc';
    ...
    layer
      .append('path')
      .attr('d', `M -6 19 A 6 6 0 0 1 6 19`)
      .attr('fill', 'none')
      .attr('stroke', stroke)
      .attr('stroke-opacity', strokeOpacity)
      .attr('stroke-width', 0.18);
    ...
    layer
      .append('path')
      .attr('d', `M -6 19 A 6 6 0 0 0 6 19`)
      .attr('fill', 'none')
      .attr('stroke', stroke)
      .attr('stroke-opacity', strokeOpacity)
      .attr('stroke-width', 0.18)
    ...
    layer
      .append('path')
      .attr('d', `M -4 5.25 A 4 4 0 0 1 4 5.25`)
      .attr('fill', 'none')
      .attr('stroke', stroke)
      .attr('stroke-opacity', strokeOpacity)
      .attr('stroke-width', 0.18);
    ...
    layer
      .append('path')
      .attr('d', `M -22 14 A 23.75 23.75 0 0 1 22 14`)
      .attr('fill', 'none')
      .attr('stroke', stroke)
      .attr('stroke-opacity', strokeOpacity)
      .attr('stroke-width', 0.18);
    ...
    layer
      ...
      .attr('y1', halfCourtLength)
      .attr('y2', halfCourtLength)
      .attr('stroke', stroke)
      .attr('stroke-opacity', strokeOpacity)
      .attr('stroke-width', 0.18);
  ...
  function draw() {
    ...
    svg.attr('viewBox', `${-halfCourtWidth} 0 ${halfCourtWidth * 2} ${halfCourtLength}`);

    const root = svg.append('g').attr('transform', `translate(0 ${halfCourtLength}) scale(1 -1)`);
    const courtLayer = root.append('g').style('pointer-events', 'none');
    const heatLayer = root.append('g');
    ...
    heatLayer
      ...
      .on('pointerleave', function () {
        hideTooltip();
        d3.select(this).attr('stroke', 'rgba(15, 23, 42, 0.18)').attr('stroke-width', 0.08);
      });
  }

  $: metricConfig = getMetricConfig();
  $: hoveredMetricValue = hoveredBin ? metricConfig.formatValue(metricConfig.getValue(hoveredBin)) : '';
...
<div bind:this={container} class="relative h-full min-h-0 w-full">
  {#if cells.length}
    <div class="flex h-full min-h-0 w-full flex-col items-center justify-start gap-4 overflow-hidden rounded-3xl bg-slate-950/70 p-4">
      ...
      <div bind:this={chartRegion} class="flex min-h-0 w-full flex-1 items-center justify-center overflow-hidden">
        <svg
    ...
    {#if hoveredBin}
      ...
      >
        ...
        <div class="mt-2 grid grid-cols-2 gap-x-3 gap-y-1 text-slate-300">
          <span>Attempts</span>
          <span class="text-right text-white">{hoveredBin.attempts.toLocaleString()}</span>
          <span>Made</span>
          <span class="text-right text-white">{hoveredBin.made.toLocaleString()}</span>
          <span>Missed</span>
          <span class="text-right text-white">{hoveredBin.missed.toLocaleString()}</span>
          <span>FG%</span>
          <span class="text-right text-white">{(hoveredBin.fgPct * 100).toFixed(1)}%</span>
        </div>
      </div>
    {/if}
  {:else}
    <div class="flex h-full min-h-[22rem] w-full items-center justify-center overflow-hidden rounded-3xl bg-slate-950/70 p-4">
      <div class="flex h-full w-full items-center justify-center rounded-2xl border border-dashed border-white/10 text-sm text-slate-400">
